...
sc_build_heatmap <- function(
  ...
  rank=25) {
  #
  if (cluster.order == FALSE & is.numeric(clusters) == FALSE) {
    cluster.sort = na.omit(unique(clusters))
    } else if (cluster.order == FALSE & is.numeric(clusters) == TRUE) {
      cluster.sort = sort(na.omit(unique(clusters)))
      } else {
        cluster.sort = cluster.order
        }
  #
  ...
  cluster.res = sapply(unique(clusters), function(cluster) {
  ...
  target.gene.z = unlist(lapply(cluster.sort, function(cluster) {
      target.gene = cluster.res[names(gene.sets.rank)[which(gene.sets.rank == cluster)], target[x], as.character(cluster)]
      names(target.gene) <- names(gene.sets.rank)[which(gene.sets.rank == cluster)]
      return(target.gene)
  }))
